...
import { verifyAndEnrollAction } from "@/app/actions/enrollment";
...
export function PaymentConfirming({ courseSlug, reference }: { courseSlug: string; reference: string }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      let result: Awaited<ReturnType<typeof verifyAndEnrollAction>>;
      try {
        result = await verifyAndEnrollAction(courseSlug, reference);
      } catch (err) {
        // A transient network/server error shouldn't permanently freeze this
        // screen — treat it like "still pending" and keep retrying until the
        // attempt cap gives the user a way out.
