...
export function PendingPaymentActions({ paymentId }: { paymentId: string }) {
  ...
  const run = (action: "approve" | "reject") => {
    ...
    startTransition(async () => {
      const result = action === "approve" ? await approvePayment(paymentId) : await rejectPayment(paymentId);
      if (!result.success) {
  ...
  if (confirming) {
    ...
    return (
      <div className="flex items-center gap-2">
        <span className={`text-xs font-semibold ${isApprove ? "text-green-700" : "text-red-700"}`}>
          {error ?? (isApprove ? "Grant access?" : "Reject this?")}
        </span>
        <button
          ...
          className={`rounded-md px-2.5 py-1 text-xs font-semibold text-white transition-colors disabled:cursor-not-allowed disabled:opacity-60 ${
            isApprove ? "bg-brand-green hover:bg-green-700" : "bg-red-600 hover:bg-red-700"
          }`}
        >
