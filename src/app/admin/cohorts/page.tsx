...
import { listAllCohorts, type CohortRow } from "@/app/actions/cohorts";
...
import { CalendarIcon, CheckCircleIcon, ClockIcon, ProgramIcon } from "@/components/icons";
...
export default async function AdminCohortsOverviewPage({
  ...
  const allCohorts = await listAllCohorts();
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Cohorts" subtitle={`${allCohorts.length} cohorts across all programs`} />

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard icon={ProgramIcon} label="Total Cohorts" value={allCohorts.length} accent="navy" />
        <StatCard icon={CheckCircleIcon} label="Enrollment Open" value={counts.open} accent="green" />
        <StatCard icon={ClockIcon} label="Ongoing" value={counts.ongoing} accent="amber" />
        <StatCard icon={CalendarIcon} label="Upcoming" value={counts.upcoming} accent="blue" />
      </div>
      ...
      <div className="rounded-2xl border border-line bg-surface">
        <div className="hidden overflow-x-auto sm:block">
          ...
          {cohorts.length === 0 && (
            <p className="py-8 text-center text-sm text-muted">No cohorts match these filters.</p>
          )}
        </div>

        <div className="space-y-2 p-4 sm:hidden">
          {cohorts.map((c) => (
            <div key={c.id} className="flex flex-col gap-2 rounded-xl bg-surface-muted p-4">
...
function CohortOverviewRow({ cohort: c }: { cohort: CohortRow }) {
  return (
    <tr className="border-b border-line last:border-0 hover:bg-surface-muted/60">
      <td className="max-w-[200px] truncate px-4 py-3 text-xs text-muted">{c.courseTitle}</td>
      <td className="px-4 py-3 text-sm font-bold text-slate-800">{c.name}</td>
      <td className="whitespace-nowrap px-4 py-3 text-xs text-muted">
        {formatDate(c.startDate)} – {formatDate(c.endDate)}
      </td>
      <td className="whitespace-nowrap px-4 py-3 text-xs text-muted">
        {c.capacity != null ? `${c.enrolledCount}/${c.capacity}` : `${c.enrolledCount} enrolled`}
      </td>
      <td className="whitespace-nowrap px-4 py-3 text-xs text-muted">{c.instructorName ?? "—"}</td>
